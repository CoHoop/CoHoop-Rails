		...
		function init(){
			...
			if(self.settings.keyNavigation){
				...
				$(document).keydown(function(e){
					...
					var chars = parseFloat(String.fromcharsCode(e.keyCode));
					if((chars > 0 && chars <= self.numberOfFrames) && (self.settings.numericKeysGoToFrames)){
						self.nextFrameID = chars;
						self.goTo(chars); //go to specified frame
					}
...
(function(a){var b="placeholder"in document.createElement("input");var c=a.browser.opera&&a.browser.version<10.5;a.fn.placeholder=function(d){var d=a.extend({},a.fn.placeholder.defaults,d),e=d.placeholderCSS.left;return b?this:this.each(function(){var b=a(this),f=a.trim(b.val()),g=b.width(),h=b.height(),i=this.id?this.id:"placeholder"+Math.floor(Math.random()*1123456789),j=b.attr("placeholder"),k=a("<label for="+i+">"+j+"</label>");d.placeholderCSS["width"]=g;d.placeholderCSS["height"]=h;d.placeholderCSS["color"]=d.color;d.placeholderCSS.left=c&&(this.type=="email"||this.type=="url")?"11%":e;k.css(d.placeholderCSS);b.wrap(d.inputWrapper);b.attr("id",i).after(k);if(f){k.hide()}b.focus(function(){if(!a.trim(b.val())){k.hide()}});b.blur(function(){if(!a.trim(b.val())){k.show()}})})};a.fn.placeholder.defaults={inputWrapper:'<span style="position:relative; display:block;"></span>',placeholderCSS:{font:"0.75em sans-serif",color:"#bababa",position:"absolute",left:"5px",top:"3px","overflow-x":"hidden",display:"block"}}})(jQuery)
